    ...
    def inject_particles(
        ...
        def append_wparray(arr, np_val, dtype):
            if arr is None:
                ...
                base = arr.numpy()
            new_np = (
                np.append(base, np_val, axis=0)
                if base.ndim == 2
                else np.append(base, np_val)
            )
            return wp.array(new_np, dtype=dtype, device=dev)

        if radius is None:
            radius = (
                self.model.particle_radius.numpy()[-1]
                if self.model.particle_radius is not None
                else self.radius
            )

        self.model.particle_q = append_wparray(
            self.model.particle_q,
            np.array([[pos[0], pos[1], pos[2]]], dtype=np.float32),
            wp.vec3,
        )
        self.model.particle_qd = append_wparray(
            self.model.particle_qd,
            np.array([[vel[0], vel[1], vel[2]]], dtype=np.float32),
            wp.vec3,
        )
        self.model.particle_mass = append_wparray(
            self.model.particle_mass, np.array([mass], dtype=np.float32), wp.float32
        )
        self.model.particle_inv_mass = append_wparray(
            self.model.particle_inv_mass,
            np.array([0.0 if mass == 0.0 else 1.0 / mass], dtype=np.float32),
            wp.float32,
        )
        self.model.particle_radius = append_wparray(
            self.model.particle_radius, np.array([radius], dtype=np.float32), wp.float32
        )
        self.model.particle_flags = append_wparray(
            self.model.particle_flags,
            np.array([int(PARTICLE_FLAG_ACTIVE.value)], dtype=np.uint32),
            wp.uint32,
        )
        self.model.particle_max_radius = float(
            max(self.model.particle_max_radius, radius)
        )
        ...
        logger.info(
            f"Injected. model.particle_count={self.model.particle_count}, state size={self.state_0.particle_count}\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_frames", type=int, default=400, help="Total number of frames."
    )
