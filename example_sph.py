...
def initialize_particles(
    particle_x: wp.array(dtype=wp.vec3), smoothing_length: float, width: float, height: float, length: float, offset: float
):
    ...
    nr_y = wp.int32(height / 4.0 / smoothing_length)          # height
    ...
    # set position with y offset
    particle_x[tid] = pos + wp.vec3(0.0, offset, 0.0)

@wp.kernel
def mesh_interaction(
    mesh: wp.uint64
):
    pass
...
class Example:
    def __init__(self, stage_path="example_sph.usd", verbose=False):
        ...
        self.gravity = -0.1
        self.n = int(
            (self.height / 4.0) * (self.width / 4.0) * (self.length / 4.0) / (self.smoothing_length**3)
        ...
        wp.launch(
            ...
            dim=self.n,
            inputs=[self.x, self.smoothing_length, self.width, self.height, self.length, self.y_offset],
